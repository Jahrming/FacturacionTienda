  ...
  <form id='formulario' name='formulario'  role='form' method='POST' enctype='multipart/form-data'>  
      <div class='container'>
        <div class='panel panel-default'>
            <div class="panel-heading" style="font-size: 20px;color:#0080FF;font-weight:bold;">CONSULTA DE CENTROS DE COSTO
              ...
              <?php if($PermisoCrear=='S')
                  {
                    echo "<button class='btn btn-primary btn-sm' title='Nuevo' style='font-size:18px;color:#0080FF;background-color:    #FFBF00;border:none' onClick='window.location=\"NuevoCentroCosto.php\"' type='button'> <span class='".$IconoCrear."'></span> </button>";
                  }?>
            </div>
        ...
        <div id='tableContainer' class='tablaContainer'>
            ...
            <thead class='fixedHeader' >
                <tr >
                    <th bgcolor='#FFBF00' style='color: #0080FF'><b><center>Id CentroCosto</center></b></th>
                    <th bgcolor='#FFBF00' style='color: #0080FF'><b><center>Código</center></b></th>
            ...
            <tbody class='scrollContent'  >
                <?php 
                    ...
                    while($registro=  mysqli_fetch_array($ResultSet))
                    {
                      ...
                      echo  
                      "<tr>
                        <td><center>".$registro['IdCentroCosto']."</center></td>   
                        <td>".$registro['CodificacionCentroCosto']."</td>                   
                        <td>".$registro['DescripcionCentroCosto']."</td>
                        <td>".$descEstado."</td>";
                      echo "<td><center>";
                      if($PermisoModificar=='S'){
                        echo "<a href=ModificarCentroCosto.php?IdCentroCosto=".$registro['IdCentroCosto']."><span class='".$IconoModificar."'></span></a>";
                      }
                      echo "&nbsp&nbsp";
                      if($PermisoEliminar=='S'){
                        echo "<a href=EliminarCentroCosto.php?IdCentroCosto=".$registro['IdCentroCosto']."><span class='".$IconoEliminar."'></span></a>";
                      }
                      echo "</td></tr>";
                    ...
                    if($contador==0)
                    {
                        echo "<h2>No existen Centro Costo</h2>";   
                    }?>
                </tbody>               
            </table>
              </div>
